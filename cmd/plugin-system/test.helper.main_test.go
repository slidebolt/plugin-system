...
// ==========================================================================
// System devices: auto-created, non-deletable, non-duplicable
// ==========================================================================
...
func TestSystemDevices_AutoCreatedOnStartup(t *testing.T) {
	...
	// Query for Time device entities
	timeEntries, err := store.Query(storage.Query{
		Pattern: app.PluginID + ".time.>",
	})
	if err != nil {
		t.Fatalf("query time entities: %v", err)
	}
	if len(timeEntries) == 0 {
		t.Error("Time device entities should be auto-created on startup, got 0")
	}

	// Query for Location device entities
	locationEntries, err := store.Query(storage.Query{
		Pattern: app.PluginID + ".location.>",
	})
	if err != nil {
		t.Fatalf("query location entities: %v", err)
	}
	if len(locationEntries) == 0 {
		t.Error("Location device entities should be auto-created on startup, got 0")
	}
}
...
func TestSystemDevices_NoDuplication(t *testing.T) {
	...
	// First plugin start
	p1 := app.New()
	if _, err := p1.OnStart(deps); err != nil {
		t.Fatalf("first OnStart failed: %v", err)
	}

	// Count entities after first start
	timeEntries1, _ := store.Query(storage.Query{
		Pattern: app.PluginID + ".time.>",
	})
	locationEntries1, _ := store.Query(storage.Query{
		Pattern: app.PluginID + ".location.>",
	})
	...
	// Second plugin start
	p2 := app.New()
	if _, err := p2.OnStart(deps); err != nil {
		t.Fatalf("second OnStart failed: %v", err)
	}
	defer p2.OnShutdown()

	// Count entities after second start - should be same
	timeEntries2, _ := store.Query(storage.Query{
		Pattern: app.PluginID + ".time.>",
	})
	locationEntries2, _ := store.Query(storage.Query{
		Pattern: app.PluginID + ".location.>",
	})

	if len(timeEntries2) != len(timeEntries1) {
		t.Errorf("Time entities duplicated: first start had %d, second start has %d",
			len(timeEntries1), len(timeEntries2))
	}
	if len(locationEntries2) != len(locationEntries1) {
		t.Errorf("Location entities duplicated: first start had %d, second start has %d",
			len(locationEntries1), len(locationEntries2))
	}
}
...
func TestSystemDevices_CannotDelete(t *testing.T) {
	...
	// Delete a time entity (simulating external deletion)
	timeKey := domain.EntityKey{Plugin: app.PluginID, DeviceID: "time", ID: "timestamp"}
	err := store.Delete(timeKey)
	if err != nil {
		t.Fatalf("Failed to delete time entity: %v", err)
	}

	// Verify entity was deleted
	_, err = store.Get(timeKey)
	if err == nil {
		t.Fatal("Entity should have been deleted")
	}

	// Trigger system device seeding again to recreate
	if err := p.SeedSystemDevices(); err != nil {
		t.Fatalf("Failed to re-seed system devices: %v", err)
	}

	// Verify entity was recreated
	_, err = store.Get(timeKey)
	if err != nil {
		t.Errorf("Time entity should be recreated after deletion, but got error: %v", err)
	}
}
...
func TestSystemDevices_TimeEntities(t *testing.T) {
	...
	expectedEntities := []string{"timestamp", "hour", "minute", "day", "week", "month", "year"}

	for _, entityID := range expectedEntities {
		key := domain.EntityKey{Plugin: app.PluginID, DeviceID: "time", ID: entityID}
		_, err := store.Get(key)
		if err != nil {
			t.Errorf("Time entity %q should exist, but got error: %v", entityID, err)
		}
	}
}
